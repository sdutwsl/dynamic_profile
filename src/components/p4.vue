...
export default {
  ...
  methods: {
    listItemClick(e) {
      ...
      const liIndex = parseInt(li.innerText.substr(-1, 1)) - 1;
      this.shows = [];
      this.shows[liIndex] = true;
      //计算列表项的中点
      ...
      if (popTop < 0) {
        ...
        this.popStyle["--popTop"]="0%";
      } else if (popBottom > window.innerHeight) {          //底部
        this.popStyle.top = -(popBottom - window.innerHeight) + "px";
        this.popStyle["--popTop"]="90%";
      } else {
        this.popStyle["--popTop"]="43%";
